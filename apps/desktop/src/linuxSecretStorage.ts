...
function getGnomeKeyringRemediationMessage(): string {
  return "Launchpad could not access GNOME Keyring to save this environment credential. Install and start GNOME Keyring, then restart Launchpad.";
}

function getKWalletRemediationMessage(): string {
  return "Launchpad could not access KWallet to save this environment credential. Enable the KDE wallet subsystem in System Settings, then restart Launchpad.";
}
