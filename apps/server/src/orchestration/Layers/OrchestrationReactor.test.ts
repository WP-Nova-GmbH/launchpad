  ...
  it("starts provider ingestion, provider command, checkpoint, and thread deletion reactors", async () => {
    ...
    runtime = ManagedRuntime.make(
      Layer.effect(OrchestrationReactor, makeOrchestrationReactor).pipe(
        ...
        Layer.provideMerge(
          Layer.succeed(OrganizationProjectCatalogRelay.OrganizationProjectCatalogRelay, {
            publishSnapshot: () => Effect.succeed(true),
            start: () => {
              started.push("organization-project-catalog-relay");
              return Effect.void;
            },
          }),
        ),
    ...
    expect(started).toEqual([
      ...
      "organization-project-catalog-relay",
    ]);
