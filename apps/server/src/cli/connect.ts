...
const authorizeCli = Effect.fn("cloud.cli.authorize")(function* (options: {
  ...
  const existing = yield* tokens.getExisting.pipe(
    Effect.catchTag("CloudCliCredentialRefreshError", () =>
      Console.log(
        "The stored Launchpad Connect credential could not be refreshed; signing in again.",
...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `t3 connect link` to authorize and enable Launchpad Connect."
    : !status.desired
      ? "Run `t3 connect link` to enable Launchpad Connect."
      : !status.linked
        ? "Start Launchpad to provision the environment link and launch its managed tunnel."
        : undefined;
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The Launchpad relay client is required for Launchpad Connect. Download and install version ${version}?`,
      initial: false,
    }),
  );
...
const logCloudDisconnectFailure = (
  ...
  Effect.logWarning("Launchpad Connect disconnect operation failed.").pipe(
    Effect.annotateLogs({
      operation,
      clearAuthorization,
...
export const reportCloudDisconnectResults = Effect.fn("cloud.cli.report_disconnect_results")(
  function* (input: {
    ...
    if (input.liveResult.status === "failed") {
      ...
      yield* Console.warn(
        "Launchpad Connect is disabled, but the running server could not stop its tunnel.\nRestart that server to stop the connector.",
      );
    } else {
      yield* Console.log("Launchpad Connect is disabled locally.");
    }
...
const linkEnvironmentForConnect = Effect.fn("cloud.cli.link_environment")(function* (options: {
  ...
  if (!publishOnly) {
    ...
    if (Option.isNone(installed)) {
      yield* Console.log("Launchpad Connect setup cancelled. The relay client was not installed.");
      return null;
    }
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription("Authorize the Launchpad Connect CLI without enabling remote access."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log("Launchpad Connect\n");
...
const connectLinkCommand = Command.make("link", {
  ...
  Command.withDescription("Authorize this environment for Launchpad Connect on next start."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (linked) {
          ...
          yield* Console.log(
            flags.publishOnly
              ? `✓ Authorized${connectedAs(linked.identity)}\n\nNext\n  Start Launchpad to publish agent activity (no managed tunnel).`
...
const connectStatusCommand = Command.make("status", {
  ...
  Command.withDescription("Show persisted Launchpad Connect and relay client state."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        if (!enabled) {
          ...
          if (!linkedNow && (yield* CliState.readCliDesiredLinkMode) === "publish_only") {
            yield* CliState.setCliDesiredCloudLink(false);
            yield* Console.log("Cancelled the pending publish-only Launchpad Connect link.");
          }
        ...
        // out of band without Launchpad Connect.
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A Launchpad Connect link is already pending. Start Launchpad to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
        yield* CliState.setCliDesiredCloudLink(true, "publish_only");
        yield* Console.log(
          "Restart Launchpad to finish authorizing this environment to publish (no managed tunnel is created).",
        );
      }),
    ),
  ),
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription("Disable Launchpad Connect while retaining the stored authorization."),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription("Disable Launchpad Connect and clear the stored CLI authorization."),
  Command.withHandler((flags) =>
...
export const connectCommand = Command.make("connect", {
  ...
  Command.withDescription("Set up Launchpad Connect for this machine."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (background) {
          ...
          yield* Console.log(
            platform === "darwin"
              ? "\n✓ Background service ready\n\nLaunchpad will stay reachable while you are logged in to this Mac."
              : "\n✓ Background service ready\n\nLaunchpad will stay reachable after you log out.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
