...
/**
 * Post-sign-in onboarding sheet for Launchpad Connect. Mobile never publishes
 * environments itself — it consumes ones published elsewhere — so this simply
 * surfaces the account's Launchpad Connect environments right after sign-in so every
...
function ConfiguredConnectOnboardingRouteScreen() {
  ...
  return (
    <View collapsable={false} className="flex-1 bg-sheet">
      {Platform.OS === "android" ? (
        <AndroidSheetHeader
          title="Set up Launchpad Connect"
        ...
        {isSignedIn ? (
          ...
          <View collapsable={false} className="rounded-[24px] bg-card p-5">
            <Text className="text-sm leading-normal text-foreground-muted">
              Sign in to your Launchpad account to set up Launchpad Connect.
            </Text>
          </View>
        )}
