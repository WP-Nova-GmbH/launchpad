...
const ConnectedClientListRow = memo(function ConnectedClientListRow({
  ...
  const personLabel = clientSession.user?.displayName ?? null;
  const primaryLabel = personLabel ? `${personLabel} · ${deviceLabel}` : deviceLabel;
...
function CloudLinkSwitch({
  ...
  disabledReason,
  onCheckedChange,
  ariaLabel = "Enable Launchpad Connect",
}: {
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const disabledReason = !isSignedIn
    ? "Sign in to Launchpad Connect to manage this environment."
    : !canManageRelay
      ? "Your session does not have permission to manage Launchpad Connect access."
      : null;
  ...
  const updateManagedTunnel = async (enabled: boolean) => {
    ...
    if (ok) {
      ...
      toastManager.add({
        type: "success",
        title: enabled
          ? "Launchpad Connect linked"
          : publishAgentActivity
            ? "Launchpad Connect tunnel disabled"
            : "Launchpad Connect unlinked",
        description: enabled
          ? "This environment is available through Launchpad Connect."
          : publishAgentActivity
            ? "The managed tunnel was removed. Agent activity publishing stays on."
            : "This environment is no longer available through Launchpad Connect.",
      });
    }
    setIsUpdating(false);
  };
  ...
  return (
    <>
      {window.desktopBridge ? (
        <SettingsRow
          title="Launchpad Connect"
          description={
            managedTunnelActive
              ? "This environment is available to your other devices through Launchpad Connect."
              : "Make this environment available to your other devices through Launchpad Connect."
          }
      ...
      <SettingsRow
        ...
        description="Send activity from this environment to your mobile clients for push notifications and Live Activities. Works without a Launchpad Connect tunnel."
        control={
          <CloudLinkSwitch
  ...
  return (
    <Empty className="min-h-52">
      ...
      <EmptyHeader>
        ...
        <EmptyDescription>
          {cloudEnabled
            ? "Click “Add environment” to pair another environment, or connect one from Launchpad Connect."
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Stop the Windows backend and run only the WSL backend. Useful if you develop entirely inside WSL and don't want a second backend process. Launchpad restarts when you change this."
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "Launchpad will restart to expose this environment over the network."
                    : "Launchpad will restart and limit this environment back to this machine."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "Launchpad will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in Launchpad until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "Launchpad will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "Launchpad will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "Launchpad will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Launchpad will restart the local backend without Tailscale Serve.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Launchpad will restart the local backend with Tailscale Serve enabled and ask
