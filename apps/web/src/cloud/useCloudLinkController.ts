...
/**
 * Drives the primary environment's Launchpad Connect link. Launchpad Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Launchpad Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[t3-connect] Could not update Launchpad Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Launchpad Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Launchpad Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Launchpad Connect before enabling this."));
        return false;
      }
