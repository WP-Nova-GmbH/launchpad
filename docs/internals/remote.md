...
Launchpad has one runtime boundary: a client talks to a Launchpad server over HTTP and WebSocket, and the server
...
└───────────────┬──────────────────────────────┘
                │ connects to one Launchpad server
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Launchpad server        │
...
One running Launchpad server instance. It owns provider availability and auth, model availability, projects
...
| `RelayConnectionTarget`   | Managed Launchpad Connect relay tunnels.                                 |
...
Tailscale is the first provider, and Launchpad manages more than discovery. When `tailscaleServeEnabled` is
...
Access answers one question: how does the client speak WebSocket to a Launchpad server? It does not answer
...
Managed Launchpad Connect relay tunnels use `RelayConnectionTarget` and are the answer when the host is
...
A Launchpad-managed `tailscale serve` mapping exposes the server on the tailnet over HTTPS, and the
...
[packages/ssh/src/tunnel.ts][sshtunnel], which resolves the target, launches or reuses the remote Launchpad
...
Launch answers a different question: how does a Launchpad server come to exist on the target machine? Keep
it separate from access.

- **Pre-existing server.** The operator already runs Launchpad and the client connects directly or through a
  tunnel.
