...
The screenshot harness runs the real mobile application against three disposable local Launchpad
...
the software keyboard, and supplies deterministic Launchpad Connect discovery rows to the real
...
1. Creates three temporary Launchpad base directories and starts a local server for each on an available
   port.
2. Creates Launchpad, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic Launchpad review diff.
...
The Pending rows use the production offline outbox and point at the real Launchpad and React fixture
