...
Use this when you want to connect to a Launchpad server from another device such as a phone, tablet, or separate desktop app.
...
run a headless server from the CLI, or have the desktop app launch Launchpad over SSH.
...
Use this when you want the desktop app to start or reuse Launchpad on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote Launchpad server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual Launchpad server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.t3/ssh-launch/<host-key>/`, starts or reuses a remote Launchpad server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. Launchpad uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Launchpad first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Launchpad uses:
...
When the Launchpad web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: Launchpad may
...
see [Keeping Launchpad in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running Launchpad in the Background](./background-service.md).
...
Hosted pairing does not proxy traffic through Launchpad. The browser still connects directly to the backend URL in the pairing link.
...
### Deregister a Launchpad Connect Environment

Open your account menu and choose **Launchpad Connect** to see every environment registered to your
account. On mobile, open **Settings** → **Launchpad Connect**. Choose **Deregister** to revoke an
environment's Launchpad Connect access, remove any managed tunnel, and free its host space.
