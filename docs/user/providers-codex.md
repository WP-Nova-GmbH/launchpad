...
This guide is for people who want to use more than one Codex account in Launchpad. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Launchpad](./install.md).
...
- both accounts can see the same Launchpad/Codex sessions
...
In Launchpad Settings, name it something obvious:
...
In Launchpad Settings, add another Codex provider:
...
Launchpad shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Launchpad treats it
