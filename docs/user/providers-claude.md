...
This guide is for people who want to use more than one Claude setup in Launchpad. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Launchpad](./install.md).
...
In Launchpad Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Launchpad uses Claude Code's normal config directory.

When you set this field, Launchpad points Claude Code at that directory with the
...
Launchpad looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where Launchpad looks.

Then add another Claude provider in Launchpad:
...
Launchpad only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Launchpad keeps separate config directories isolated
...
Add or edit a Claude provider in Launchpad Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Launchpad stores the value as a server secret and does not
...
Launchpad does not need a special Claude Code Router provider. Treat the router as a Claude
