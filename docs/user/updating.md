# Keeping Launchpad in Sync

The Launchpad web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Launchpad shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                    |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the Launchpad Linux background service. Select the button and leave Launchpad open while it prepares, tests, restarts, and reconnects.                          |
| **Update the desktop app** | Open the Launchpad desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current Launchpad server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. Launchpad does not update connected
...
If the server instead runs as the Launchpad background service, update the service on the host and
...
See [Running Launchpad in the Background](./background-service.md) for install, status, and removal
commands.
